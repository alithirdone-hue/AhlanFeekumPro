...
* Check the `ConnectionStrings` in `appsettings.json` files under the `AhlanFeekumPro.Blazor` and `AhlanFeekumPro.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p e90b936e-bbdd-400d-b3da-1bb56e7ecf9d
```

> `e90b936e-bbdd-400d-b3da-1bb56e7ecf9d` is the password of the certificate, you can change it to any password you want.
...
Run `AhlanFeekumPro.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `AhlanFeekumPro.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `AhlanFeekumPro.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
