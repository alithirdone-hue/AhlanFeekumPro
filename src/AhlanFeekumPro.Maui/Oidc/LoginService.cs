...
using AhlanFeekumPro.Maui.Messages;
...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(AhlanFeekumProConsts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(AhlanFeekumProConsts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(AhlanFeekumProConsts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(AhlanFeekumProConsts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(AhlanFeekumProConsts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(AhlanFeekumProConsts.OidcConsts.RefreshTokenKeyName);
    }
}